...
if($action == 'show'){ 
    ...
    $sql = "select r.*,s.Supplier from tblremain r ,tblsupplier s where s.AID=r.SupplierID ".$a." 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="select r.AID from tblremain r,tblsupplier s where s.AID=r.SupplierID ".$a." 
        order by AID desc";
...
if($action == 'save'){
    ...
    $sql="insert into tblpurchasereturn (RemainID,SupplierID,Qty,Price,Amt,Date) values 
    ('{$aid}','{$supplierid}','{$qty}','{$price}','{$amt}','{$dt}')";  
    if(mysqli_query($con,$sql)){
        
        $rid = mysqli_insert_id($con);
        $sql_detail = "insert into tblsupplierdetail (SupplierID,PurchaseID,Amt,Date,ReturnID) 
        values ('{$supplierid}','{$aid}','-{$amt}','{$dt}','{$rid}')";
        mysqli_query($con,$sql_detail);
        
        purchase_return($aid,$remainqty);
        
        save_log($_SESSION["eadmin_username"]."သည် Purchase Return အား Save လုပ်သွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
