...
if($action == 'show'){  
    ...
    $a = "";
    if($search != ''){   
        $a = " where Name like '%$search%' or PhoneNo like '%$search%' or Address like '%$search%' ";
    }      
    $sql="select * from tblcustomer  ".$a." 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="select AID from tblcustomer  ".$a." 
        order by AID desc";
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblcustomer where AID=$aid";
            ...
            $out.="<div class='modal-body'>
                <input type='hidden' id='aid' name='aid' value='{$row['AID']}'/>                                              
                    <div class='form-group'>
                        ...
                        <input type='text' class='form-control border-success' id='name' name='name' value='{$row['Name']}'>
                    </div>
                    <div class='form-group'>
                        <label for='usr'> ဖုန်းနံပါတ် :</label>
                        <input type='text' class='form-control border-success' id='phno' name='phno' value='{$row['PhoneNo']}'>
                    </div> 
                    <div class='form-group'>
                        <label for='usr'> နေရပ်လိပ်စာ :</label>
                        <input type='text' class='form-control border-success' id='address' name='address' value='{$row['Address']}'>
                    </div> 
                    <div class='form-group'>
                        <label for='usr'> အီးမေးလ် :</label>
                        <input type='email' class='form-control border-success' id='email' name='email' value='{$row['Email']}'>
                    </div>                                
                </div>
...
if($action == 'update'){
    ...
    $sql = "update tblcustomer set Name='{$name}',PhoneNo='{$phno}',
    Address='{$address}',Email='{$email}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["eadmin_username"]."သည် customer အား update လုပ်သွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'delete'){
    $aid = $_POST["aid"];
    $sql = "delete from tblcustomer where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["eadmin_username"]." သည် customer အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "CustomerReport-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '<head><meta charset="utf-8"></head>
        <table >  
            <tr>
                <td colspan="5" align="center"><h3>Customer</h3></td>
            </tr>
